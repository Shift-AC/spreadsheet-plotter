...
- `msp`: A frontend of `sp` that invokes it to generate multiple data series and render them with pluggable backends. Today `msp` ships with a `gnuplot` backend for traditional preview/PDF workflows and an `echarts` backend that generates HTML output.
...
- Runtime dependencies (need to be in `$PATH`)
    ...
    `msp`   | `sp` (provided by this project), plus `gnuplot` and `ps2pdf` when using the `gnuplot` backend
...
`msp`, instead, leverages `sp` as a data processing tool to generate multiple data series, normalizes the request into a backend-neutral plot specification, and then hands it to the selected renderer backend.
...
Like `sp`, `msp` has the `-m` option that determines the mode of `msp`. The default value is `plot`, which prepares the data series and renders them with the selected backend. With `-m prepare`, `msp` prepares the datasheet files but does not execute the renderer; instead it prints the resolved backend-neutral request, the generated artifact paths, and a short render-plan summary. With `-m dry-run`, `msp` stops even earlier and only prints the resolved backend-neutral request without invoking `sp` or any renderer.
